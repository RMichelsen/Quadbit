...
namespace Quadbit::VkUtils {
	...
	inline uint32_t GetDynamicUBOAlignment(const QbVkContext& context, uint32_t structSize) {
		// Calculate required alignment based on minimum device offset alignment
		uint32_t minAlignment = static_cast<uint32_t>(context.gpu->deviceProps.limits.minUniformBufferOffsetAlignment);
		auto alignment = structSize;
		if (minAlignment > 0) {
