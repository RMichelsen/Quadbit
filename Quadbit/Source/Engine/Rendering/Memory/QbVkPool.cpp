...
QbVkPool::QbVkPool(VkDevice device, const int32_t memoryTypeIndex, const VkDeviceSize size, QbVkMemoryUsage usage) : 
	device_(device), capacity_(size), allocatedSize_(0), memoryTypeIndex_(memoryTypeIndex), nextBlockId_(0), data_(nullptr), memoryUsage_(usage) {
...
void QbVkPool::Free(QbVkAllocation& allocation) {
	...
	if(current == nullptr) {
		printf("QbVkAllocator: Trying to free an unknown allocation (%i) in pool %p", allocation.id, allocation.pool.get());
		return;
	}
