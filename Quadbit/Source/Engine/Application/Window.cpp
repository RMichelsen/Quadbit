...
Window::Window(HINSTANCE hInstance, int nCmdShow, WNDPROC windowProc) {
	...
	if(!SetupWindow(nCmdShow, windowProc)) {
		printf("Failed to setup win32 Window\n");
		return;
	}
	// We'll set up console and inputhandler in the window class for now
	if(!SetupConsole()) {
		printf("Failed to setup the console\n");
		return;
	}
	if(!InputHandler::Initialize()) {
		printf("Failed to setup the input handler\n");
		return;
	}
}
