...
namespace Quadbit {
	...
	QbVkPBRMaterial PBRPipeline::ParseMaterial(const tinygltf::Model& model, const tinygltf::Material& material) {
		...
		mat.descriptorAllocator = pipeline_->descriptorAllocator_;
		return mat;
	}
	...
	QbVkDescriptorSetsHandle PBRPipeline::WriteMaterialDescriptors(const QbVkPBRMaterial& material) {
		...
		pipeline_->BindResource(descriptorSetsHandle, "baseColorMap", baseColorHandle);
		pipeline_->BindResource(descriptorSetsHandle, "metallicRoughnessMap", metallicRoughnessHandle);
		pipeline_->BindResource(descriptorSetsHandle, "normalMap", normalHandle);
		pipeline_->BindResource(descriptorSetsHandle, "occlusionMap", occlusionHandle);
		pipeline_->BindResource(descriptorSetsHandle, "emissiveMap", emissiveHandle);
