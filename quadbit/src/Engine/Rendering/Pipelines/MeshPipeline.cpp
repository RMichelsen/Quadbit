...
namespace Quadbit {
	...
	void MeshPipeline::CreatePipeline() {
		...
		auto bindingDescription = QbVkVertex::GetBindingDescription();
		auto attributeDescriptions = QbVkVertex::GetAttributeDescriptions();
		...
		pipelineLayoutInfo.pSetLayouts = &context_.resourceManager->materialDescriptorSetLayout_;
	...
	const QbVkRenderMeshInstance* MeshPipeline::CreateInstance(eastl::vector<QbVkRenderDescriptor>& descriptors, eastl::vector<QbVkVertexInputAttribute> vertexAttribs,
		...
		externalInstances_.push_back(eastl::make_unique<QbVkRenderMeshInstance>());
		...
		if (!descriptors.empty()) {
			...
			eastl::vector<VkDescriptorSetLayout> layouts(context_.swapchain.images.size(), renderMeshInstance->descriptorSetLayout);
		...
		eastl::array<VkDynamicState, 2> dynamicStates{
	...
	//	eastl::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT, descriptorSetLayout_);
	//	VkDescriptorSetAllocateInfo descSetallocInfo = VkUtils::Init::DescriptorSetAllocateInfo();
